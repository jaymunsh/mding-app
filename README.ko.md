...
HTML 파일은 의도적으로 읽기 전용입니다. mding은 가져온 단일 `.html` / `.htm` 파일을 같은 로컬 작업공간에 저장하고, 실행 가능한 내장 요소를 제거한 뒤 sandbox iframe 안에서 렌더링합니다. `<pre class="mermaid">`, `<div class="mermaid">`, `<code class="language-mermaid">` 같은 정적 Mermaid 블록은 iframe이 로드되기 전에 mding이 SVG로 렌더링합니다.

HTML 편집과 외부 로컬 asset 폴더 관리는 현재 범위에서 제외합니다. 원격 URL, data URL, 인라인 CSS는 브라우저가 접근할 수 있는 경우 표시될 수 있습니다.
