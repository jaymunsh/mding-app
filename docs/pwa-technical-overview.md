...
mding은 오프라인 속도를 위해 Service Worker의 정적 자산 캐시와 한 번 불러온 Mermaid·Shiki 모듈 캐시는 유지한다. 대신 현재 문서에서 생성된 렌더링 결과는 파일 전환 시 폐기한다.

- HTML 미리보기를 닫으면 iframe 실행을 중단하고 빈 문서로 교체해 기존 DOM과 script 참조를 해제한다.
- HTML 문서 생성에는 취소 신호를 전달해 이전 파일의 비동기 결과가 새 미리보기에 반영되지 않도록 한다.
- Mermaid SVG는 컴포넌트가 사라질 때 즉시 제거하고, 렌더링 전후에 취소 여부를 확인한다.
- 여러 Mermaid 다이어그램의 테마 감시는 개별 `MutationObserver` 대신 하나의 공유 감시자를 사용한다.
- Markdown과 HTML 미리보기는 문서 ID를 기준으로 재생성해 이전 검색 표시와 React 렌더링 트리를 유지하지 않는다.

따라서 문서를 다시 열 때 문서별 Markdown 변환과 Mermaid SVG 생성은 다시 수행될 수 있지만, 라이브러리 파일을 재다운로드하지는 않는다. 브라우저 엔진 자체의 기본 메모리는 유지될 수 있으므로 이 처리는 브라우저 프로세스 전체를 최소화하기보다 파일 전환 후 불필요한 문서 자원이 누적되는 것을 방지하는 데 목적이 있다.
...
Apple 플랫폼에서 말하는 무료 개발자 계정의 7일 제한은 네이티브 앱을 직접 사이드로드할 때의 코드 서명 만료 문제다. mding처럼 Safari에서 홈 화면에 추가하는 PWA에는 그 제한이 적용되지 않는다.

Safari/WebKit의 ITP에는 사용자 상호작용이 없는 일부 script-writable 웹사이트 저장소를 7일 후 정리하는 정책이 있지만, 홈 화면에 추가한 웹 앱의 first-party domain은 이 특정 7일 cap의 예외로 문서화되어 있다. 따라서 iOS 홈 화면 PWA를 “7일 뒤 자동 삭제되는 앱”으로 보면 안 된다. 다만 브라우저 관리 저장소라는 사실은 변하지 않으므로, 사용자가 앱이나 사이트 데이터를 삭제하거나 저장 공간 압박으로 정리되는 위험은 계속 고려해야 한다.
